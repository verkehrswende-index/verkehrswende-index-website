...
const Map = dynamic(() => import("components/feature-map"), { ssr: false });
...
const Analysis = ({ areaId, areaConfig, id, className, data }) => {
  ...
  return (
    <div className={className}>
      <h2>
        {config.title} in{" "}
        <Link href={`/gebiete/${areaId}`}>{areaConfig.name}</Link>
      </h2>
      ...
      {hasMap && !showMap && (
        <p>
          <Button onClick={() => setShowMap(true)} variant="info">
            Zeige Karte{" "}
            {featuresDownloadSize > 0
              ? `(> ${Math.round(featuresDownloadSize / 1024)}kb)`
              : ""}
          </Button>
          &nbsp; (je nach Größe der Stadt werden viele Daten geladen. Kann auf
          älteren Computern oder im Mobilfunk Probleme bereiten)
        </p>
      )}
      {showMap && (
        <Map area={areaId} analysis={config.name} config={config.map} />
      )}
    </div>
  );
};
