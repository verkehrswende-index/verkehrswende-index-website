...
export async function fetchFeaturesDownloadSize(area: string, analysis: string) : Promise<number> {
...
export async function fetchFeatures(area: string, analysis: string) : Promise<GeoJSON.FeatureCollection> {
    const response = await fetch(
        config.remoteDataURI + `areas/${area}/analysis/${analysis}/features.json`
    );
