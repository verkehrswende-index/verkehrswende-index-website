  ...
  return (
    <Layout>
      ...
      <small className="text-muted">
        {Object.keys(data)
          .map((tag) =>
            tag.match(/^name:(?!de$)[a-z]{2}$/) ? data[tag] : null
          )
      ...
      <Row className="mt-4 mb-5">
        <Col xs={8} sm={3} className="mb-3">
          <Score
            showMaxScore={true}
            score={data.score}
            oldScore={data.score1Y}
          />
        </Col>
        <Col xs={12} sm={9}>
          <strong>{__("Einwohner*innen")}:</strong>{" "}
      ...
      <div className={styles["analysis-previews"]}>
        {Object.keys(registeredAnalysis).map((slug) => (
          <AnalysisPreview
            ...
            slug={slug}
            area={area}
            title={registeredAnalysis[slug].title}
            results={analysis[slug].results}
            results1y={analysis[slug].results1y}
            className="mb-5"
          />
        ))}
      </div>
